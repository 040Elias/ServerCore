...
public class StoreCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!p.hasPermission("servercore.store")) {
            p.sendMessage(plugin.messages().component("no-permission", Map.of()));
            SoundUtil.playError(plugin, p);
            return true;
        }

        List<?> raw = plugin.getConfig().getList("store.lines");
        if (raw == null || raw.isEmpty()) {
            p.sendMessage(TextUtil.toComponent("&cNo store message configured. Add 'store.lines' to config.yml."));
            return true;
        }
        ...
        String sound = plugin.getConfig().getString("store.sound", "");
